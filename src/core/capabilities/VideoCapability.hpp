...
public:
    ...
    virtual void playVideo(
        const QUrl& source,
        std::function<void(int playbackId, const QString& error)> callback
    ) = 0;
    ...
    virtual void openCamera(
        ...
        int frameRate,
        std::function<void(int captureId, const QString& error)> callback
    ) = 0;
    ...
    virtual void startRecording(
        ...
        std::function<void(int recordingId, const QString& error)> callback
    ) = 0;
    ...
    virtual void captureFrame(
        int captureId,
        std::function<void(const QByteArray& frameData, FrameFormat format)> callback
    ) = 0;
    ...
    virtual int subscribeToFrames(
        int captureId,
        std::function<void(const QByteArray& frameData, FrameFormat format, qint64 timestamp)> callback
    ) = 0;
     ...
     * @return Subscription ID for unsubscribe
     */
    virtual int subscribeToPlaybackState(
        int playbackId,
        std::function<void(PlaybackState state)> callback
    ) = 0;
