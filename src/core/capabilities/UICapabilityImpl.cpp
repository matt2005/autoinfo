...
QString UICapabilityImpl::extensionId() const { return extension_id_; }
bool UICapabilityImpl::isValid() const { return is_valid_; }
void UICapabilityImpl::invalidate() { is_valid_ = false; }
...
bool UICapabilityImpl::registerMainView(const QString& qmlPath, const QVariantMap& metadata) {
    ...
    if (!registrar) { qWarning() << "UIRegistrar not set; cannot register main view"; return false; }
...
bool UICapabilityImpl::registerWidget(const QString& qmlPath, const QVariantMap& metadata) {
    ...
    auto* registrar = manager_->uiRegistrar();
    if (!registrar) { qWarning() << "UIRegistrar not set; cannot register widget"; return false; }
    registrar->registerComponent(extension_id_, "widget", qmlPath, metadata);
    return true;
}

void UICapabilityImpl::showNotification(const QString& title, const QString& message, int duration, const QString& icon) {
    if (!is_valid_) return;
    Q_UNUSED(duration); Q_UNUSED(icon);
    manager_->logCapabilityUsage(extension_id_, "ui", "showNotification", QString("%1: %2").arg(title, message));
}

void UICapabilityImpl::updateStatusBar(const QString& itemId, const QString& text, const QString& icon) {
    if (!is_valid_) return;
    Q_UNUSED(icon);
    manager_->logCapabilityUsage(extension_id_, "ui", "updateStatusBar", QString("%1: %2").arg(itemId, text));
}
