...
void NavigationExtension::start() {
    ...
    if (uiCap) {
        ...
        qInfo()
            << "Navigation: Registered main view with path: qrc:/navigation/qml/NavigationView.qml";
    }
    ...
    if (locationCap) {
        location_subscription_id_ = locationCap->subscribeToUpdates(
            [this](const QGeoCoordinate& coord) { updateCurrentLocation(coord); });
...
void NavigationExtension::registerConfigItems(core::config::ConfigManager* manager) {
    ...
    routingModeItem.properties["options"] =
        QStringList{"Fastest", "Shortest", "Eco", "Avoid Highways", "Avoid Tolls"};
    ...
    voiceLanguageItem.properties["options"] =
        QStringList{"English (UK)", "English (US)", "French", "German", "Spanish", "Italian"};
...
void NavigationExtension::setupEventHandlers() {
    ...
    eventCap->subscribe("navigation.navigateTo",
                        [this](const QVariantMap& data) { handleNavigateToCommand(data); });

    eventCap->subscribe("navigation.cancel",
                        [this](const QVariantMap& data) { handleCancelNavigationCommand(data); });

    eventCap->subscribe("navigation.setDestination",
                        [this](const QVariantMap& data) { handleSetDestinationCommand(data); });

    eventCap->subscribe("navigation.searchLocation",
                        [this](const QVariantMap& data) { handleSearchLocationCommand(data); });
...
void NavigationExtension::handleNavigateToCommand(const QVariantMap& data) {
    ...
    qInfo() << "Starting navigation to:" << address << "(" << latitude << "," << longitude << ")";
...
void NavigationExtension::handleSearchLocationCommand(const QVariantMap& data) {
    ...
    if (networkCap && networkCap->isOnline()) {
        ...
        QString searchUrl =
            QString("https://nominatim.openstreetmap.org/search?q=%1&format=json&limit=5")
