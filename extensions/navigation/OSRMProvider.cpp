...
void OSRMProvider::calculateRoute(const QGeoCoordinate& start, const QGeoCoordinate& end,
...
void OSRMProvider::handleRouteResponse() {
    ...
    for (const QJsonValue& coord : coordinates) {
        ...
        if (point.size() >= 2) {
            route.coordinates.append(QGeoCoordinate(point[1].toDouble(), point[0].toDouble()));
        }
    }
    ...
    if (!legs.isEmpty()) {
        ...
        for (const QJsonValue& stepVal : steps) {
            ...
            if (location.size() >= 2) {
                step.location = QGeoCoordinate(location[1].toDouble(), location[0].toDouble());
            }
